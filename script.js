...
class Equation {
  ...
  switchOperator() {
    if (this.lhs instanceof Equation && this.rhs instanceof Equation) {
      ...
      if (this.op == OPERATOR_ADD) this.op = OPERATOR_SUBTRACT
      if (this.op == OPERATOR_MULTIPLY) this.op = OPERATOR_DIVIDE
    }
  }
  ...
  toString() {
    if (
      ...
      (this.lhs.op == OPERATOR_MULTIPLY || this.lhs.op == OPERATOR_DIVIDE) &&
      (this.op == OPERATOR_ADD || this.op == OPERATOR_SUBTRACT)
    )
      this.lhs.parentheses = false
    if (
      this.rhs instanceof Equation &&
      (this.rhs.op == OPERATOR_MULTIPLY || this.rhs.op == OPERATOR_DIVIDE) &&
      (this.op == OPERATOR_ADD || this.op == OPERATOR_SUBTRACT)
    )
    ...
    if (
      ...
      this.op == OPERATOR_SUBTRACT &&
      this.rhs.op == OPERATOR_ADD &&
      this.op == OPERATOR_DIVIDE &&
      this.rhs.op == OPERATOR_MULTIPLY &&
      Math.random() >= 0.5
    ) {
      this.rhs.parentheses = false
      if (this.op == OPERATOR_SUBTRACT && this.rhs.op == OPERATOR_ADD) this.rhs.op = OPERATOR_SUBTRACT
      if (this.op == OPERATOR_DIVIDE && this.rhs.op == OPERATOR_MULTIPLY) this.rhs.op = OPERATOR_DIVIDE
